...
def slack_interactive_endpoint():
    ...
    if action_id == "snooze_1d":
        ...
        if not cfg.not_stale_label:
            ...
            return (
                "Error: Not configured to apply 'not stale' label.",
                200,
            )  # 200 to ack to Slack
        ...
        labels_url = (
            f"{GITHUB_API_BASE_URL}/repos/{owner}/{repo}/issues/{pr_number}/labels"
        )
        ...
        try:
            ...
            print(
                f"Successfully labeled PR {pr_url} with '{cfg.not_stale_label}'. Status: {response.status_code}"
            )
